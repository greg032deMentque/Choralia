...
|---|---|---|
| `.claude/CLAUDE.md` (ce fichier) | tout le depot | protocole de travail, checklist qualite, routage agents, regles transverses |
| `ChoraleBack/CLAUDE.md` | `ChoraleBack/` | specifique .NET : conventions de test NUnit, enums, prompt d'audit |
| `ChoralFront/CLAUDE.md` | `ChoralFront/` | specifique Angular web : structure, tokens, zones, contrat `/auth/Me` |
| `ChoraleMobile/CLAUDE.md` | `ChoraleMobile/` | specifique Ionic Angular : pages, onglets, Capacitor |
...
2. **La deduplication ne se fait qu'entre ces trois fichiers.** Le `CLAUDE.md` global
   (`~/.claude/CLAUDE.md`) est local a un poste : un autre ordinateur, un agent cloud ou un
...
Declinaison ChoraleHelper des « Criteres de qualite » du CLAUDE.md global. Le recouvrement avec
le global est **volontaire** : ce fichier est versionne, le global est local a un poste. Sur une
autre machine, en agent cloud, ou pour un subagent qui ne charge que le CLAUDE.md projet, cette
...
`ChoralFront/CLAUDE.md` § Regle tests. Motif : le runner est le builder natif
...
ChoraleMobile/         -> Ionic Angular, cible membres et chefs de pupitre (dossier sans
                          code d'application pour l'instant — CLAUDE.md seul)
